...
def analyze_ping(target="8.8.8.8", count=3):
    try:
        result = subprocess.run(["ping", "-c", str(count), target], capture_output=True, text=True, check=True, timeout=5)
...
def extract_feature_ratios():
    ...
    # Store temporary lists to average later
    packet_loss_vals = []
    jitter_vals = []
    dns_vals = []
    hop_counts = []
    ...
    for _ in range(3):
        ...
        packet_loss_vals.append(ping["packet_loss"])
        jitter_vals.append(ping["latency_jitter"])
        dns_vals.append(dns_time)
        hop_counts.append(trace["hop_count"])
        per_hop_rtts.append(trace["per_hop_rtt"])

        time.sleep(1)  # Small pause between readings to avoid hammering

    # Compute averages over 3 samples
    values = {
        "packet_loss": round(statistics.mean(packet_loss_vals), 2),
        "latency_jitter": round(statistics.mean(jitter_vals), 2),
        "dns_resolve_time": round(statistics.mean(dns_vals), 2),
        "hop_count": round(statistics.mean(hop_counts), 2),
        "per_hop_rtt": round(statistics.mean(per_hop_rtts), 2)
    }
...
# === Classification ===
def run_live_classification():
    conf.iface = get_top_network_interface()  #for scapy
    ensure_baseline_files_exist()
    load_rolling_buffer()

    if not os.path.exists(MODEL_PATH):
        print("[x] Trained model not found.")
        return

    model = XGBClassifier()
    model.load_model(MODEL_PATH)
    print("[✓] ISP Model loaded. Starting real-time classification...\n")

    label_names = model.classes_ if hasattr(model, "classes_") else None
    ...
    while True:
        ...
        if label_names is not None:
            ...
            top3 = [(f"Class {i}", round(probs[i]*100, 2)) for i in top3_indices]

        print(f"[{datetime.now().strftime('%H:%M:%S')}] Top 3 Predictions: " + ", ".join([f"{lbl} ({conf}%)" for lbl, conf in top3]))
        time.sleep(1)
